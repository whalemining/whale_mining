let coins = 0;
    ...
    window.onload = function() {
        const telegramId = getUrlParameter('user_id');  // Get the Telegram user ID from the URL
        if (telegramId) {
            window.location.href = `/telegram-auth/?telegram_id=${telegramId}`;
        } else {
    ...
    avatarImage.addEventListener('touchstart', function(event) {
        ...
        if (touchPoints >= 1) {
            ...
            fetch('/', { // Assuming your URL for the tap_coin view
                method: 'POST',
                headers: {
                    'X-CSRFToken': getCSRFToken(), // Add the CSRF token to the header
